...
### Примеры диалога (TODO для отчёта)

> TODO: Вставьте сюда 2–3 примера диалога с ботом в режиме `/mode_gpt` (текстом или скриншотами).
>
> TODO: Повторите те же запросы в режиме `/mode_template` и вставьте ответы рядом, чтобы было видно сравнение «LLM vs шаблоны» на одинаковых пользовательских данных.

Пример структуры (заполнить своими данными):
...
- Разработана автономная диалоговая система в формате Telegram‑бота (long polling).
- Реализована генерация осмысленного текста не по шаблонам: режим `/mode_gpt` (YandexGPT).
- Добавлен baseline на шаблонах для сравнения: режим `/mode_template`.
- В `README.md` есть инструкции по сборке, развёртыванию (Docker/VPS) и использованию.
- В `README.md` есть место (TODO) для вставки примеров диалога и сравнения на пользовательских данных.
